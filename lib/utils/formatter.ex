defmodule SmartTracer.Utils.Formatter do
  ...
  def format({:trace_ts, _pid, :call, {module, func_name, args}, erl_time}, nil) do
  ...
  def format({:trace_ts, _pid, :call, {module, func_name, args}, erl_time}, custom_formatter) do
    if record?(), do: Recorder.record({:call, {module, func_name, args, erl_time}})

    apply(custom_formatter, :action, [:call, {module, func_name, args}])
  end

  def format(
        {:trace_ts, _pid, :return_from, {module, func_name, arity}, return_value, erl_time},
        nil
      ) do
    if record?(),
  ...
  def format(
    ...
    apply(custom_formatter, :action, [:return, {module, func_name, arity, return_value}])
  end
